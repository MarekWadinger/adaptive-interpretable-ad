...
def plot_limits_3d(
    ...
    **kwargs,
    ...
    fig = make_subplots(
        ...
        cols=2,
        specs=[
            [{"colspan": 2, "rowspan": 3, "type": "scatter3d"}, None],
            [None, None],
            [None, None],
            [{}, {}],
        ],
...
def add_thresholds(
    ...
    fig.add_trace(
        go.Scatter(
            x=thresh_high.index,
            y=([max(yaxis_range)]) * len(thresh_high),
    ...
    fig.add_trace(
        go.Scatter(
            x=thresh_low.index,
            y=([min(yaxis_range)]) * len(thresh_low),
            line_color="rgba(100,100,100, 0)",
            name="Threshold",
            legendgroup="thresh",
            showlegend=False,
...
def plot_compare_anomalies(
    ...
    **kwargs,
    ...
    for row, anomaly in enumerate(anomalies, start=1):
        ...
        fig.update_yaxes(
            title_text=f"{ser.name}<br>{anomaly}",
            title_standoff=0,
            row=row,
            col=1,
        )
...
if __name__ == "__main__":
    ...
    df_out = (
        pd.read_json("data/output/dynamic_limits.json", lines=True)
        .set_index("time")
        .dropna()
    )

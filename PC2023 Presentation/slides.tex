...
\begin{frame}{Drawbacks of Existing Solutions}
    \begin{figure}[htpb]
        \begin{center}
            \includegraphics[width=\linewidth]{/Users/marekwadinger/PyProjects/river/ilustrate/online.pdf}
        \end{center}
    \end{figure}
\end{frame}
...
\begin{frame}{Inversion of CDF}
    ...
    \end{figure}
    \begin{align*}
        \ui{x}{l} & = F_{X}(1 - q; \bar x_n, s_n)^{-1}\text{,}\nonumber \\
        \ui{x}{u} & = F_{X}(q; \bar x_n, s_n)^{-1}\text{.}\nonumber
    \end{align*}
\end{frame}
...
\begin{frame}{Online Anomaly Detection Workflow}
    ...
    \begin{algorithmic}[1]
        ...
        \IF {\eqref{case:normal} \OR ${\sum_{y\in Y}y}/{n(Y)} > q$}
    ...
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \foreach \y [count=\i] in {0, -2} {
                    ...
                    \node[circle,draw, minimum size=1.5cm] (\labels[\i-1]) at (0, \y) {$\label$};
            ...
            \node[circle,draw, minimum size=1.5cm] (G_C) at  (6, -2) {$G_C$};
            ...
            (MQTT-t) -- (G_C-t) node [pos=0.5,above=0.2cm] {S: {\color{black}{$T, SOC, V$}}};
...
\begin{frame}{Utilize Existing Infrastructure}
    \begin{figure}
        ...
        \begin{tikzpicture}
            ...
            \node[circle,draw, minimum size=1.5cm] (V) at (0, -4) {$V$};
            % Connect middle circle to the circle on the right
            \node[circle,draw, minimum size=1.5cm] (G_C) at  (6, 0) {$G_C$};
            \draw[-latex, signal] (MQTT) -- (G_C);

            % Add subscribe lines
            \draw[-latex, subscribe] (MQTT) edge[bend right,draw=none] coordinate[at start](MQTT-b) coordinate[at end](G_C-b) (G_C)
            edge[bend left,draw=none] coordinate[at start](MQTT-t) coordinate[at end](G_C-t) (G_C)
            (MQTT-t) -- (G_C-t) node [pos=0.3,above=0.7cm] {S: {\color{black}{$T, SOC, V, \color{red}{A}$}}};
            ...
            \draw[latex-, subscribe] (ICDF-b) -- (MQTT-b) node [pos=0.7,below=0.7cm] {S: {\color{black}{$T, SOC, V$}}};
